...
class CloudinaryService:
    ...
    def list_resources(
        ...
        Returns:
            Dict with "resources" list, "next_cursor" (if more pages exist)
        """
        if not self.enabled:
    ...
    def is_cloudinary_url(self, url: str) -> bool:
        """Check if a URL belongs to our Cloudinary account.

        Args:
            url: URL to check

        Returns:
            True if the URL is from Cloudinary and matches our cloud
        """
        if not url:
            return False
        parsed = urlparse(url)
        return "cloudinary.com" in parsed.hostname or "res.cloudinary.com" in (parsed.hostname or "")
    ...
    def extract_public_id_from_url(url: str) -> str | None:
        ...
        Args:
            url: Cloudinary URL (e.g. "https://res.cloudinary.com/cloud/image/upload/v123/magnific/image/model/id")
        ...
        # Cloudinary URL format: .../upload/{version}/{public_id}.{format}
        # or .../upload/{public_id}.{format}
        try:
            parsed = urlparse(url)
            path = parsed.path
            # Find the segment after /upload/ or /upload/v{digits}/
            import re
